    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            top_p_thresh = torch.abs(torch.nn.functional.softmax(next_tokens_scores, dim=-1) - 0.0) < 0.000001
            top_p_thresh_sum = top_p_thresh.sum(dim=-1)
            _top_p_len = next_tokens_scores.shape[-1] - top_p_thresh_sum
            top_p_len += (_top_p_len, )
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            top_p_thresh = torch.abs(torch.nn.functional.softmax(next_token_scores, dim=-1) - 0.0) < 0.000001
            top_p_thresh_sum = top_p_thresh.sum(dim=-1)
            _top_p_len = next_token_scores.shape[-1] - top_p_thresh_sum
            top_p_len += (_top_p_len, )
